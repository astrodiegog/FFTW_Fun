...
OneD_Tests = fObj['OneDimensionalTests']
OneD_attrs = OneD_Tests.attrs

x_arr = OneD_attrs['x_arr']
Nx = x_arr.size
dx = x_arr[1] - x_arr[0]
kx_arr_c = OneD_attrs['kx_arr_c']
kx_arr_r = OneD_attrs['kx_arr_r']

kx_np_c = np.fft.fftfreq(Nx, dx)
kx_np_r = np.fft.rfftfreq(Nx, dx)
...
OneD_testkeys = OneD_Tests.keys()
    
for testkey in OneD_testkeys:
    OneD_Test = OneD_Tests[testkey]
    print(testkey)
   
    fName = Path(testkey + ".png")
    fPath = cwd / fName

    y_arr = OneD_Test.get('y_arr_Real')[:] + 1.j * OneD_Test.get('y_arr_Imag')[:]

    FFT_c2c_arr = OneD_Test.get('FFT_c2c_Real')[:] + 1.j * OneD_Test.get('FFT_c2c_Imag')[:]
    FFT_r2c_arr = OneD_Test.get('FFT_r2c_Real')[:] + 1.j * OneD_Test.get('FFT_r2c_Imag')[:]

    FFT_analytic_c2c_arr = OneD_Test.get('FFT_analytic_c2c_Real')[:] + 1.j * OneD_Test.get('FFT_analytic_c2c_Imag')[:]
    FFT_analytic_r2c_arr = OneD_Test.get('FFT_analytic_r2c_Real')[:] + 1.j * OneD_Test.get('FFT_analytic_r2c_Imag')[:]

    iFFT_c2c_arr = OneD_Test.get('iFFT_c2c_Real')[:] + 1.j * OneD_Test.get('iFFT_c2c_Imag')[:]
    iFFT_c2r_arr = OneD_Test.get('iFFT_c2r')[:]
